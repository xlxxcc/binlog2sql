...
import com.caiye.binlogsql.vo.ColumnItemDataVo;
...
public class SqlGenerateTool {
    ...
    private static DateTimeFormatter dateTimeFormater = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
     ...
     */
    public static List<String> updateSql(TableVo tableInfo, List<UpdateHandler.Pair> rowPairs, String comment) {
        List<String> sqls = new ArrayList<>();
        for (UpdateHandler.Pair rowPair : rowPairs) {
    ...
    private static String getUpdateSetCondition(RowVo rowVo) {
        ...
        for (ColumnItemDataVo columnItemDataVo : rowVo.getValue()) {
            ...
            if (item != null) {
                setCondition.add(item);
            }
        }
        return String.join(" , ", setCondition);
    }
    ...
    public static String getStringByColumnValue(ColumnItemDataVo itemDataVo) {
        ...
        switch (itemDataVo.getColumn().getJdbcType()) {
            case BIT:
                return toBinaryString((BitSet) itemDataVo.getValue());
            case TINYINT:
            case SMALLINT:
            case INTEGER:
            case BIGINT:
    ...
    private static String toBinaryString(BitSet bits) {
        final StringBuilder buffer = new StringBuilder("b'");
        if (bits.length() > 0) {
            IntStream.range(0, bits.length()).mapToObj(i -> bits.get(i) ? '1' : '0').forEach(buffer::append);
